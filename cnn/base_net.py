...
_norm_layer = nn.BatchNorm1d
#todo ffn res to hightlight the recent trend
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.dropblock(self.conv1(x)) if self.drop else self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)

        out = self.dropblock(self.conv2(out)) if self.drop else self.conv2(out)
        out = self.bn2(out)
